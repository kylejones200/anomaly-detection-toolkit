...
class TestFeatureExtractor:
    ...
    def test_extract_from_dataframe(self):
        ...
        extractor = FeatureExtractor(rolling_windows=[5, 10])
        data = pd.DataFrame({"temp": np.random.randn(100), "pressure": np.random.randn(100)})
        features = extractor.extract(data)
...
class TestAlertSystem:
    ...
    def test_check_thresholds(self):
        ...
        thresholds = {"temperature": {"warning": 80.0, "critical": 90.0, "failure": 100.0}}
...
class TestPredictiveMaintenanceSystem:
    ...
    def test_process_with_fitted_models(self):
        ...
        # Fit models
        rul_estimator.fit(train_features, np.random.uniform(0, 1000, len(train_features)))
        failure_classifier.fit(train_features, np.random.randint(0, 2, len(train_features)))
    ...
    def test_process_with_anomaly_detector(self):
        """Test processing with integrated anomaly detector."""
        detector = ZScoreDetector(n_std=2.0)
        system = PredictiveMaintenanceSystem(anomaly_detector=detector)
        ...
        # Fit detector
        detector.fit(data.values.reshape(-1, 1))
        ...
        assert results["anomaly_prediction"] in [-1, 1]
...
class TestUtilityFunctions:
    ...
    def test_add_rolling_statistics(self):
        ...
        result = add_rolling_statistics(
            df, feature_cols=["sensor1"], asset_id_col="asset_id", cycle_col="cycle", window=3
        )
