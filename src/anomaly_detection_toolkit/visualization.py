...
def plot_pca_boundary(
    ...
    """
    Visualize PCA boundary in 2D projection.
    ...
    n_components_plot : int, default=2
        Number of components to plot (must be at least 2).
    ...
    # Plot PCA mean
    if detector.mean_ is not None and n_components_plot <= detector.mean_.shape[0]:
        ax.scatter(
            detector.mean_[0],
            detector.mean_[1],
            c=colors["primary"],
    ...
    ax.set_xlabel(
        f"Principal Component 1 ({detector.get_explained_variance_ratio()[0]:.2%} variance)"
    )
    ax.set_ylabel(
        f"Principal Component 2 ({detector.get_explained_variance_ratio()[1]:.2%} variance)"
    )
...
def plot_sensor_drift(
    sensor_data: Union[np.ndarray, pd.Series],
